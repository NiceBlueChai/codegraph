...
fn rust_trait_impl_project() -> TempDir {
    let dir = tempfile::tempdir().expect("temp project");
    fs::create_dir_all(dir.path().join("src")).expect("create src");
    fs::write(
        dir.path().join("Cargo.toml"),
    ...
    fs::write(
        ...
        concat!(
            ...
            "impl crate::types::Render for Mine { fn render(&self) -> i32 { self.x } }\n",
        ),
    )
    .expect("write consumer");
...
#[test]
fn affected_follows_rust_trait_implementation() {
    let dir = rust_trait_impl_project();
    let output = run_codegraph(
        &["affected", "src/types.rs", "--json", "--depth", "5", "--filter", "src/consumer.rs"],
        dir.path(),
    );
